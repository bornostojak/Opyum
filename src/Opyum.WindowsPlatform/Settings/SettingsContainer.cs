    ...
    public class SettingsContainer : IDisposable
    {
        ...
        private void updateShortcuts(IEnumerable<IShortcutKeyBinding> shortcuts)
        {
            ...
            if (shortcuts != null && shortcuts.Count() > 0)
            {
                ...
                var groupbypresent = shortcuts.GroupBy(c => old.ContainsKey(c.Command) ? 
                    (old[c.Command].ShortcutKeys.SequenceEqual(c.ShortcutKeys) &&
                    old[c.Command].Global == c.Global &&
                    old[c.Command].IsDisabled == c.IsDisabled ? 2 : 1) : 0, c => c, (gr, keys) => new { Group = gr, Shortcuts = keys });
                ...
                foreach (var item in groupbypresent)
                {
                    ...
                    if (item.Group == 1)
                    {
                        ...
                        {
                            // Upate shortcut, global and disabled
                            old[i.Command].UpdateShortcut(i.ShortcutKeys);
                            old[i.Command].Global = i.Global;
                            old[i.Command].IsDisabled = i.IsDisabled;
                        }
                    }
                ...
                //find owerlapping shortcuts that can prematurely resolve into other shortucts (you cant use "Ctrl + K" as hortcuts if "Ctrl + K, Ctrl + P" exists)
        ...
        protected class ShortcutDictonayComparer : IEqualityComparer<Keys[]>
        {
